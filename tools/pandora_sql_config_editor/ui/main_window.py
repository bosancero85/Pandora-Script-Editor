...
from core.backup import backup_sqlite, backup_mariadb
...
class MainWindow(QMainWindow):
    ...
    def _build_actions(self):
        ...
        self.act_open_sqlite = QAction("📂 Datei öffnen (SQLite)…", self)
    ...
    def _on_open_sqlite(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "SQLite-Datenbank öffnen", "", "SQLite DB (*.db *.sqlite *.sqlite3);;Alle Dateien (*)"
        )
        if not path:
            return
        try:
    ...
    def _run_backup(self):
        if self.db.backend == "sqlite" and self.db.sqlite_path:
    ...
    def _on_save_as(self):
        ...
        if self.db.backend == "sqlite":
            target, _ = QFileDialog.getSaveFileName(
                self, "SQLite-Datenbank speichern unter", "",
                "SQLite DB (*.db *.sqlite *.sqlite3);;Alle Dateien (*)"
            )
            if not target:
                return
            try:
                self.db.conn.commit()
                shutil_copy_target = Path(target)
                import shutil
                shutil.copy2(self.db.sqlite_path, shutil_copy_target)

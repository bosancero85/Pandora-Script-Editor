...
class MainWindow(QMainWindow):
    ...
    def _get_system_python(self):
        """Ermittelt den echten Python-Interpreter des Systems.

        Läuft der Script Editor als normales .py-Skript, ist sys.executable
        bereits der korrekte Python-Interpreter. Läuft er hingegen als
        gepackte PyInstaller-.exe (sys.frozen == True), zeigt sys.executable
        auf die .exe des Editors selbst - ein F5-Start würde dann den Editor
        erneut starten (inkl. Login-Dialog), statt das geladene Skript
        auszuführen. In diesem Fall wird stattdessen nach 'python'/'python3'/
        'py' im PATH gesucht."""
        ...
        QMessageBox.warning(
            ...
            "Es konnte kein System-Python (python/python3/py) im PATH "
            "gefunden werden. Skripte können daher nicht per F5 ausgeführt "
            "werden.\n\nBitte stelle sicher, dass Python installiert und "
    ...
    def run_script(self):
        ...
        python_interpreter = self._get_system_python()
        ...
        self.process.start(python_interpreter, ["-u", editor._file_path])
    ...
    def _launch_external_tool(self, script_path, extra_args=None, friendly_name="Tool"):
        ...
        - Python-Quellskript (*.py)   -> wird mit dem System-Python-
          Interpreter gestartet (siehe _get_system_python(); wichtig, wenn
          der Script Editor selbst als gepackte .exe läuft).
        ...
        try:
            if is_python_script:
                ...
                if not python_interpreter:
                    return
                args = [python_interpreter, script_path] + list(extra_args or [])
            else:
